...
class SocialShare
{
    ...
    {
        $url = config('social-share.services.facebook.uri') . $this->url;
    ...
    public function twitter()
    {
        if (is_null($this->title)) {
            $this->title = config('social-share.services.twitter.text');
        }

        $base = config('social-share.services.twitter.uri');
    ...
    {
        if (is_null($this->title)) {
            $this->title = config('social-share.services.reddit.text');
        }

        $base = config('social-share.services.reddit.uri');
    ...
    {
        if (is_null($this->title)) {
            $this->title = config('social-share.services.telegram.text');
        }

        $base = config('social-share.services.telegram.uri');
    ...
    {
        $url = config('social-share.services.whatsapp.uri') . $this->url;
    ...
    {
        $base = config('social-share.services.linkedin.uri');
        $mini = config('social-share.services.linkedin.extra.mini');
    ...
    {
        $url = config('social-share.services.pinterest.uri') . $this->url;
    ...
    {
        if (is_null($this->title)) {
            $this->title = config('social-share.services.viber.text');
        }

        $url = config('social-share.services.viber.uri') . $this->title . $this->url;
    ...
    {
        $fontAwesomeVersion = config('social-share.fontAwesomeVersion', 5);

        $this->rememberRawLink($provider, $url);

        $this->html .= trans("social-share::social-share-fa$fontAwesomeVersion.$provider", [
            'url' => $url,
